/*Program:    famtree.c
  ...
  Purpose:    This is lab 1 for cs360. It takes in information about differnt people on stdin. The informatiion will
              describe the person's familial relations, and sometimes the sex of the person or the sex of the person 
			  can be inferred from the information. The program stores all people in a RB tree, and all of that person's 
			  children in a Dllist. It then uses a form of a DFS to look for someone who is their own descendant. If no
			  cyles are found it will use a modified BFS to print the people making sure to not print parents before kids*/
...
/*This is a DFS that will return a 1 if if finds a cyle(a person who is their own descendant)*/
...
int main(int argc, char **argv)
{
	...
	while( get_line(is) >= 0 )
	{
		...
		if( is->NF != 0)
		{
			...
			if( strcmp( is->fields[0],"PERSON") == 0 )
			{
				...
				//Create a key made of the names of the person seperated by a space and then 
			...
			else if( strcmp(is->fields[0],"MOTHER") == 0)
			{
				...
				/*For the FATHER_OF/MOTHER_OF lines of input get the name of the suppossed child and see 
				 * if it is the tree. If not add it, then make sure that if the new persons father/mother 
				 * is set to the current person. If it is in the tree make sure that if its father/mother is 
				 * already set make sure the current persons name matches the name of the child's father/mother
				 * .if so  ignore, otherwise throw an error*/
	...
	//perform a BFS to make sure you print parents before children
	while( !dll_empty(toprint) )
	{
		...
		if(pg->prntd == 0)
		{
			...
			{
				//if it could not  printed put it back on the list
				dll_append(toprint, new_jval_s(pg->name));
			}
		}
	}
