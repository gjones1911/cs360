...
//			printf("the %d kid added is %s \n",j, (*edges[j]).name);
	...
	/*the below will print all of the nodes in initial cast range*/
//	printf("The nodes that are in the initial cast range are...\n");
...
//	}
	...
	/*the below will print all of the nodes and their adjacent list*/
//	for(i = 0; i < player_cnt ; i++)
//	{
//		printf("The node %d is %s and it's kids are....\n",i ,(*nodes[i]).name);
//
//		/*Now print the adjacent list of this node*/
...
//	printf("the best amount of healing possible is %d\n",Ip->best_heal);

//	printf("the best path is as follows\n");
/*
	for( i = 0; i < Ip->best_path_length;i++)
	{
		p1 = (Ip->best_path[i]);
		if(p1 = NULL) printf("yes it is NULL\n");
		printf("the path length is %d\n",Ip->best_path_length);
		printf("%s ,%d, %d\n", p1->name, i+1, p1->healing);
	}
*/
	for(i = 0; i < Ip->best_path_length ;i++)
	{
